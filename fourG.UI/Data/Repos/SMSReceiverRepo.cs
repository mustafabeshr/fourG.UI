...
namespace fourG.UI.Data.Repo
{
    public class SMSReceiverRepo
    {
        ...
        public async Task<List<SMSReceiver>> GetListAsync(int id, string mobileNo, int count = 1000)
        {
            ...
            #endregion
            var sql = $"SELECT TOP({count}) * FROM [dbo].[sms_receiver] {where} ORDER BY row_id desc";
        ...
        public async Task<List<SMSReceiver>> GetListAsync(string message, string mobileNo, int count = 1000)
        {
            var where = string.Empty;
            var parameters = new List<SqlParameter>();
            #region Build Where Clause
